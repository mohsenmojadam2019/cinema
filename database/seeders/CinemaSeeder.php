...
class CinemaSeeder extends Seeder {
 public function run(): void {
  ...
  $images=['home-concept.png','seat-selection-concept.png','admin-dashboard-concept.png']; foreach($events as $i=>[$title,$summary,$cat]){$e=Event::create(['organization_id'=>$org->id,'category_id'=>$cats[$cat]->id,'title'=>$title,'slug'=>Str::slug($title).'-'.$i,'summary'=>$summary,'description'=>'اطلاعات کامل اثر، عوامل، داستان و تجربه تماشای آن در سینماپلاس.','type'=>$cat==='theater'?'theater':'cinema','duration'=>105,'age_rating'=>$i%2?'+۱۲':'همه سنین','is_featured'=>$i<3,'status'=>'published']);$image=public_path('images/concepts/'.$images[$i%count($images)]);if(is_file($image))$e->addMedia($image)->preservingOriginal()->toMediaCollection('poster'); for($day=0;$day<7;$day++)Show::create(['event_id'=>$e->id,'venue_id'=>$venues[$i%3]->id,'starts_at'=>now()->addDays($day)->setTime(18+$i%3,30),'ends_at'=>now()->addDays($day)->setTime(20+$i%3,20),'price'=>$cat==='theater'?350000:220000,'vip_price'=>420000]);}
